...
export function runMemoryTool(name: string, input: Record<string, unknown> = {}, options: { recordings?: any[] } = {}) {
  if (!TOOL_NAMES.includes(name)) {
    throw new Error(`Unknown memory tool: ${name}`);
  }

  const recordings = sortRecordings(options.recordings ?? []);
...
function sortRecordings(recordings: any[]) {
  return [...recordings].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
}
...
function getDailyLoop(recordings: any[], input: Record<string, unknown>) {
  const date = normalizeDate(input.date) || todayIsoDate();
...
function getRecordings(recordings: any[], input: Record<string, unknown>) {
...
function getRecording(recordings: any[], input: Record<string, unknown>) {
...
function search(recordings: any[], input: Record<string, unknown>) {
...
function getEnergyPatterns(recordings: any[], input: Record<string, unknown>) {
  const filtered = filterRecordings(recordings, dateScopedInput(input, 30));
  const buckets = {
    givers: new Map<string, { item: string; count: number }>(),
    sappers: new Map<string, { item: string; count: number }>(),
    recoverers: new Map<string, { item: string; count: number }>(),
  };
...
function getBalanceSnapshot(recordings: any[], input: Record<string, unknown>) {
  ...
  const snapshot = Object.fromEntries(
    CENTERS.map((center) => [center, { count: 0, examples: [] as any[] }]),
  );
...
function filterRecordings(recordings: any[], input: Record<string, unknown> = {}) {
...
function dateScopedInput(input: Record<string, unknown> = {}, defaultDays: number) {
...
function completionSummary(morning: any, evening: any) {
  const morningItems = [
    ...(morning?.structured_note?.priorities ?? []),
    ...(morning?.structured_note?.todos ?? []).map((todo: any) => todo.text),
  ];
  const eveningItems = [
...
function toAgentRecording(recording: any) {
  return {
    id: recording.id,
...
function searchableText(recording: any) {
  const note = recording.structured_note ?? {};
  return [
...
function matchedExcerpt(recording: any, queryTokens: string[]) {
  const candidates = [
...
function addEnergyMatches(
  bucket: Map<string, { item: string; count: number }>,
  text: string,
  patterns: RegExp[],
  seenKeys: Set<string>,
) {
...
function increment(bucket: Map<string, { item: string; count: number }>, item: string, providedKey?: string) {
  const key = providedKey ?? normalizeForComparison(item);
  if (!key) return;

  const current = bucket.get(key) ?? { item, count: 0 };
  current.count += 1;
  bucket.set(key, current);
}

function mapToCounts(bucket: Map<string, { item: string; count: number }>) {
...
function completionMatches(expected: unknown, evidence: unknown) {
...
function recordingLocalDate(recording: any) {
...
function clampLimit(value: unknown, defaultLimit: number, maxLimit: number) {
